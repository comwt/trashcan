...
case `echo ${JULIAN} | cut -c1-2` in
  00 ) JULIAN=`echo ${JULIAN} | cut -c3` ;;
  0* ) JULIAN=`echo ${JULIAN} | cut -c2-3` ;;
  *  ) ;;
esac

case `echo ${LASTPURGE} | cut -c1-2` in
  00 ) LASTPURGE=`echo ${LASTPURGE} | cut -c3` ;;
  0* ) LASTPURGE=`echo ${LASTPURGE} | cut -c2-3` ;;
  *  ) ;;
esac
...
#                (( NUMFLD += 1 ))
  ...
  -rest)  #--------------------------------------------------------------
          ...
          CNT=`ls -A ${TC} | wc -l`
          #- Uncomment the lines below if your architecture does not support
          #  the '-A' option for ls, and comment out the line above.
          #-------------------------------------------------------------------
          #CNT=`ls -a ${TC} | wc -l`
          #(( CNT -= 2 ))  #-- Decrement by 2 for "." and ".."
  ...
  -purge  )  #--  Delete files older than trash.conf 'KeepDays' specified.
          ...
          if [[ ${LASTPURGE} -eq ${JULIAN} ]]; then
            ...
            #--------------------------------
            sed "s/Current = ${CURRENT}/Current = ${OJULIAN}/" ${TCON} > ${TCON2}
            mv -f ${TCON2} ${TCON}
            ...
            #----------------------------------------------------------
            {
              sed "s/LastPurge = ${OLASTPURGE}/LastPurge = ${OJULIAN}/" ${TCON} > ${TCON2}
              mv -f ${TCON2} ${TCON}
            }
  ...
  -keep  )  #-- Configure number of days to keep trash
          ...
          clear
          banner Trash 2>/dev/null
          banner Config 2>/dev/null
          echo ""
          ...
          if [[ ${ANSW} == "Y" || ${ANSW} == "y" ]]; then
            ...
            if [[ ${NEWKEEP} -ne ${KEEPDAYS} ]]; then
              ...
              #-------------------------
              sed "s/LastPurge = ${OLASTPURGE}/LastPurge = 0/" ${TCON2} > ${TCON}
              rm ${TCON2}
